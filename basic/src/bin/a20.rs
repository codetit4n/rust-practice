...
fn print_power_action(state: PowerState) {
    use PowerState::*; //Now we don't need to do PowerState::Off,etc everytime.
    match state {
        Off => println!("truning off"),
...
fn main() {
    ...
    if user_input_status.is_ok() {
        //is_ok() is a Result function
        match PowerState::new(&buffer) {
            //we need to convert buffer to borrowed
